...
LEFT_TO_CHEST_CENTER_Y = LEFT_BODY_START + 4
LEFT_TO_TORSO_CENTER_Y = LEFT_BODY_START + 7
DEFAULT_Y_OFFSET = -0.035


def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Apply a residual-specific you-vs-i left-body calibration experiment.")
...
def main() -> None:
    ...
    for row in classification_payload.get("classifications", []):
        ...
        adjusted_mean[4] += left_body_y_offset
        adjusted_mean[7] += left_body_y_offset
        ...
        if should_apply:
            ...
            adjusted_sequence[:, LEFT_TO_CHEST_CENTER_Y] += left_body_y_offset
            adjusted_sequence[:, LEFT_TO_TORSO_CENTER_Y] += left_body_y_offset
            adjusted_logits = _predict_logits(
                model=model,
            ...
            adjusted_top3, adjusted_probabilities = _probabilities_from_logits(adjusted_logits, index_to_label)
            ...
            updated["top1_logit"] = _safe_float(adjusted_logits[int(np.argmax(adjusted_logits))])
            updated["correct"] = int(str(updated["predicted_label"]).strip().lower() == str(updated["token"]).strip().lower())
            updated["you_i_left_body_calibration"] = {
                "left_body_y_offset": _safe_float(left_body_y_offset),
    ...
    updated_payload["you_i_left_body_calibration"] = {
