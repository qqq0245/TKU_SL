...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Diagnose pose-local landmark graph sensitivity on cached exact spans.")
...
def _pose_graph_indices() -> list[int]:
    ...
    indices = list(range(left_xyz + right_xyz, left_xyz + right_xyz + pose_xyz))
    indices.extend(range(pose_mask_start, pose_mask_start + pose_nodes))
    return indices
...
def main() -> None:
    ...
    pose_indices = _pose_graph_indices()
    ...
    for span in spans:
        ...
        ablated_sequence[:, pose_indices] = 0.0
        ...
        rows.append(
            {
                ...
                "changed_after_pose_graph_ablation": changed,
    ...
    summary = {
        ...
        "row_count": len(rows),
        "changed_after_pose_graph_ablation_count": int(sum(row["changed_after_pose_graph_ablation"] for row in rows)),
        "changed_tokens": dict(changed_counter),
        "pose_graph_feature_columns": pose_indices,
    }
    output_payload = {
