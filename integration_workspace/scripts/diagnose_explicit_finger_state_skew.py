...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Diagnose explicit finger-state feature sensitivity on cached exact spans.")
...
def main() -> None:
    ...
    explicit_section = feature_spec["landmark_sections"]["explicit_finger_states"]
    explicit_start = int(explicit_section["start"])
    explicit_end = int(explicit_section["end"])
    explicit_indices = list(range(explicit_start, explicit_end))
    ...
    for span in spans:
        ...
        ablated_sequence[:, explicit_start:explicit_end] = 0.0
        ...
        rows.append(
            {
                ...
                "changed_after_explicit_finger_ablation": changed,
    ...
    summary = {
        ...
        "row_count": len(rows),
        "changed_after_explicit_finger_ablation_count": int(sum(row["changed_after_explicit_finger_ablation"] for row in rows)),
        "changed_tokens": dict(changed_counter),
        "explicit_finger_feature_columns": explicit_indices,
    }
    output_payload = {
